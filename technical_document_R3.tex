...
推定されたパラメータ$a$，$b$について，その相関が強い場合には，推定が不安定である可能性がある．そこでこのパラメータ間の相関係数$\textit{r_{ab}}$を推定時に算出したヘッセ行列$\textbf{\textit{H}}$の逆行列である共分散行列$\textbf{\textit{Cov}$を算出し、相関行列$\textbf{\textit{Cor}$を求めることで導出する．
\begin{eqnarray}
  {\bf Cov} = {\bf H}^{-1}
...
推定された再生産パラメータ$a$，$b$のもとで，$F=0$とした時の初期資源量（処女資源量）$B_0$とそれに対応する加入量$R_0$に対し，親魚量が初期資源量の20％のとき（$0.2 B_0$）に推定された再生産関係式から導出される加入量を$R_h$とした時の，$R_0$に対する$R_h$の比であるスティープネス$h$を算出する。この定義に従えばBH型，RI型のスティープネスはそれぞれ以下のように計算される．
