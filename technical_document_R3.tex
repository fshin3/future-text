...
推定された再生産パラメータ$a$，$b$のもとで，$F=0$とした時の初期資源量（処女資源量）$B_0$とそれに対応する加入量$R_0$に対し，親魚量が初期資源量の20％のとき（$0.2 B_0$）に推定された再生産関係式から導出される加入量を$R_h$とした時の，$R_0$に対する$R_h$の比であるスティープネス$h$を算出する。この定義に従えばBH型，RI型のスティープネスはそれぞれ以下のように計算される．このとき$S\!P\!R_0$は$F=0$としたときの加入量あたり産卵親魚量である．

\subsubsection*{ベバートン・ホルト型再生産関係(BH)のときのスティープネス$h$}
\begin{eqnarray}
  h=\frac{a \: 0.2 \:  B_0} {1 + b \: 0.2 \: B_0} / R_0 = \frac{a \: 0.2 \: S\!P\!R_0}{(1 + b \: 0.2 \: B_0)}
  \label{BHh}
\end{eqnarray}

\subsubsection*{リッカー型再生産関係(RI)のときのスティープネス$h$}
\begin{eqnarray}
  h= a \: 0.2 \: B_0 \exp{-b \: 0.2 \: B_0} / R_0 = a \: 0.2 \: S\!P\!R_0 \exp{(-b \: 0.2 \: B_0)}
  \label{RIh}
\end{eqnarray}
