...
\title{{\normalsize FRA-SA-2021-ABCWGxx-yy}\\
...
\section{令和2年度版からの変更点}
\begin{itemize}
\item 再生産関係式で推定されるパラメータ間の相関について追記
\item 再生産関係のスティープネスについての説明を追加(\ref{calch}) 
\end{itemize}
...
推定されたパラメータ$a$，$b$について，その相関が強い場合には，推定が不安定である可能性がある．そこでこのパラメータ間の相関係数$r_{ab}$を，パラメータ推定時に算出したヘッセ行列$\textbf{H}$の逆行列である共分散行列$\textbf{Cov}$を用いることで，相関行列$\textbf{Cor}$を求めて導出する．
\begin{eqnarray}
  {\bf Cov} = {\bf H}^{-1}
  \end{eqnarray}
\begin{eqnarray}
  {\bf Cor} = \left( \begin{array}{cc} 
			1 & r_{ab} \\
			r_{ba} & 1 
			\end{array}
			\right)
			= \left( \begin{array}{cc} 
			\sigma_{aa}/s_{aa} & \sigma_{ab}/\sigma_{ab} \\
			\sigma_{ba}/\sigma_{bb} & \sigma_{bb}/\sigma_{bb} 
			\end{array}
			\right)
			= {\bf Cov}
\end{eqnarray}
...
\subsection{スティープネスの算出\label{calch}}
推定された再生産パラメータ$a$，$b$のもとで，$F=0$とした時の初期資源量（処女資源量）$B_0$とそれに対応する加入量$R_0$に対し，親魚量が初期資源量の20％のとき（$0.2 B_0$）に推定された再生産関係式から導出される加入量を$R_h$とした時の，$R_0$に対する$R_h$の比であるスティープネス$h$を算出する。この定義に従えばBH型，RI型のスティープネスはそれぞれ以下のように計算される．このとき$SPR_0$は$F=0$としたときの加入量あたり産卵親魚量である．

\subsubsection*{ベバートン・ホルト型再生産関係(BH)のときのスティープネス$h$}
\begin{eqnarray}
  \mathrm{h}=\frac{a \: 0.2 \:  B_0} {1 + b \: 0.2 \: B_0} / R_0 = \frac{a \: 0.2 \: SPR_0}{(1 + b \: 0.2 \: B_0)}
  \label{BHh}
\end{eqnarray}

\subsubsection*{リッカー型再生産関係(RI)のときのスティープネス$h$}
\begin{eqnarray}
  \mathrm{h}= a \: 0.2 \: B_0 \exp{-b \: 0.2 \: B_0} / R_0 = a \: 0.2 \: SPR_0 \exp{(-b \: 0.2 \: B_0)}
  \label{RIh}
\end{eqnarray}

スティープネスは再生産関係式における密度補償効果の程度を示す．0.2から1の間の値を取り，0.2の時は密度補償効果がないことを，1の時は親魚量に関わらず一定の加入量が得られることを意味する．

また，ホッケー・スティック型の再生産関係式の場合，上記の定義に従うと$0.2B_0 > b$において全て$h=1$となってしまうため，Punt et al. (2014)\cite{HSsteepness}の定義に従い以下の通りに算出する．

\subsubsection*{ホッケー・スティック型再生産関係(HS)のときのスティープネス$h$}
\begin{eqnarray}
  \mathrm{h}= 1 - \frac{b}{B_0}
  \label{HSh}
\end{eqnarray}

スティープネスの値が小さい場合，資源の回復を強く念頭に置く必要がある．
...
\bibitem{HSsteepness} Punt AE, Smith ADM, Smith DC, Tuch GN, Klaer NL (2014) Selecting relative abundance proxies for $B_{MSY}$ and $B_{MEY}$. ICES Journal of Marine Science 71:469--483.
