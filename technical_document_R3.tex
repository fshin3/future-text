...
推定されたパラメータ$a$，$b$について，その相関が強い場合には，推定が不安定である可能性がある．そこでこのパラメータ間の相関係数$r_{ab}$を，パラメータ推定時に算出したヘッセ行列$\textbf{H}$の逆行列から共分散行列$\textbf{Cov}$を求め，相関行列$\textbf{Cor}$を導出する．
\begin{eqnarray}
  {\bf H} = {\bf Cov}^{-1}
  \end{eqnarray}
\begin{eqnarray}
...
ここで，$\textbf{D}$は$ii$成分が$1/\sigma_i$となる以下の対角行列である．
\begin{eqnarray}
